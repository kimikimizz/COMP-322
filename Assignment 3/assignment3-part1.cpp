...
class AbstractPlayer
{
    ...
    public:
        friend class BlackJackGame;
        // error when adding const (virtual bool isDrawing() const = 0;)
...
class HumanPlayer: public AbstractPlayer
{
    ...
    public:
        ...
        void announce() {
            if (isBusted()) cout << "Player busts." << endl << "Casino wins." << endl;
...
class BlackJackGame
{
    ...
    public:
        ...
        BlackJackGame() { m_deck = Deck(); };
        void reset() {
            m_casino.hand.clear();
            m_player.hand.clear();
            m_player.to_draw_or_not_to_draw = 'y';
            m_deck = Deck();
        }
        void player_draw_card() {
            do {
                ...
                if (m_player.hand.getTotal() == 21) break;
            }
        ...
        void who_wins() {
            ...
            else if (m_casino.hand.getTotal() == m_player.hand.getTotal()) {
        ...
        void play() {
            ...
            m_casino.total_games++;
            // for debug
            cout << "Current casino winning rate: " << ((float)m_casino.games_won / (float)m_casino.total_games) << endl;
            // Reset for new round
            reset();
        }
};
